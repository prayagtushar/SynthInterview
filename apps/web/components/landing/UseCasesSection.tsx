...
const useCases = [
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-full h-full">
        ...
        <path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20" className="opacity-40" />
  ...
  },
  {
    ...
    sector: "RETENTION",
    description: "Deep-dive into senior-level architectural decision making. Verify trade-offs during internal transition cycles.",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-full h-full">
...
export default function UseCasesSection() {
  return (
    <section id="use-cases" className="section-container bg-[#030303] border-t border-white/5 py-32 md:py-48 overflow-hidden">
        ...
        >
          <motion.header variants={fadeUpVariants} className="mb-32 text-center flex flex-col items-center">
            <div className="section-badge mb-8 gap-2.5">
               ...
               <span className="leading-none translate-y-[0.5px]">Vertical Solutions</span>
            </div>
            ...
            <p className="mt-10 text-white/70 max-w-2xl text-lg md:text-xl leading-relaxed mx-auto group-hover:text-white transition-colors">
              From fast-moving startups to global enterprises—Synth adapts to your unique hiring needs.
            </p>
          </motion.header>
            ...
            {useCases.map((useCase) => (
              ...
              >
                <div className="p-10 lg:p-14 rounded-[2.5rem] border border-white/[0.05] bg-white/[0.01] hover:bg-white/[0.025] hover:border-white/20 transition-all duration-700 relative overflow-hidden h-full flex flex-col shadow-xl">
                  ...
                  <div className="absolute inset-0 opacity-[0.02] group-hover:opacity-[0.06] transition-opacity duration-1000 pointer-events-none" 
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div className={`w-16 h-16 rounded-[1.5rem] ${useCase.accent} border border-white/10 flex items-center justify-center text-white mb-10 shadow-lg p-4 transition-all duration-700 group-hover:scale-110 group-hover:border-white/30 group-hover:shadow-[0_0_30px_rgba(255,255,255,0.05)]`}>
                       {useCase.icon}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 mt-auto">
                       {useCase.benefits.map(benefit => (
                          <span key={benefit} className="px-5 py-2 rounded-full border border-white/10 bg-white/[0.05] text-[10px] font-bold uppercase tracking-[0.2em] text-white/40 group-hover:text-white/60 transition-all">{benefit}</span>
                       ))}
                    </div>
                  </div>
                </div>
