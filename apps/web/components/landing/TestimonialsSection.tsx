...
export default function TestimonialsSection() {
  return (
    <section id="testimonials" className="section-container bg-[#030303] border-t border-white/5 py-32 md:py-48 overflow-hidden">
      <div className="section-inner relative z-10 px-6 sm:px-10 max-w-7xl">
        ...
        >
          <div className="section-badge mb-8 gap-2.5">
             ...
             <span className="leading-none translate-y-[0.5px]">Ecosystem Signals</span>
          </div>
          <h2 className="section-title text-gradient !text-4xl md:!text-7xl leading-[1.1]">
             Loved by <br /> <span className="font-serif-italic">engineering</span> teams.
          </h2>
        </motion.header>
            ...
            >
              <div className="space-y-8">
                <div className="flex gap-1.5">
                   {[1,2,3,4,5].map(i => <Star key={i} size={12} fill="white" stroke="none" className="opacity-10 group-hover:opacity-40 transition-opacity" />)}
                </div>
              ...
              <div className="flex items-center gap-6 border-t border-white/5 pt-10 mt-12">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.3em] text-white/20">
                    <span className="group-hover:text-white/50 transition-colors">{testimonial.title}</span>
                    <span className="w-1 h-1 rounded-full bg-white/20" />
                    <span className="text-white/50 group-hover:text-white/80 transition-colors">{testimonial.company}</span>
                  </div>
                </div>
              </div>
            </motion.div>
