...
export default function SessionView() {
	...
	const runCode = useCallback(async () => {
		...
		try {
			const qid = questionData
				? (questionData as any).id || questionData.title
				: undefined;

			const res = await fetch(`${API_BASE}/sessions/${sessionId}/run-code`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ code, language, question_id: qid }),
			});
			...
			if (isConnected) {
				if (data.passed === data.total && data.total > 0) {
					sendEvent('candidate_signal', {
						signal: 'All tests passed! I am ready to advance.',
					});
				} else {
	...
	// Track pending injection: set when a new question arrives, cleared after injection
	const pendingInjectionRef = useRef<string | null>(null);

	// When questionData changes to a new question, mark it as pending and clear stale terminal
	useEffect(() => {
		if (!questionData) return;
		const qid = (questionData as any).id || questionData.title;
		if (lastInjectedQuestionId.current === qid) return;
		// New question — mark pending and clear stale results
		pendingInjectionRef.current = qid;
		setRunResult(null);
		setShowTerminal(false);
	}, [questionData]);

	// Inject code whenever we enter PROBLEM_DELIVERY and there's a pending question
	useEffect(() => {
		if (currentState !== 'PROBLEM_DELIVERY') return;
		if (!questionData) return;
		const qid = (questionData as any).id || questionData.title;
		// Inject if this question hasn't been injected yet (pending or first load)
		if (
	...
	return (
		<div
			className={`min-h-[100dvh] relative overflow-hidden bg-[#030303] text-slate-100 flex font-sans selection:bg-white selection:text-black ${
			...
			{webcamStream && (
				<div className='fixed bottom-8 right-8 z-[60] overflow-hidden rounded-none border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.5)] w-56 h-40 bg-zinc-950 group/webcam transition-all duration-300 hover:scale-105 hover:border-white/50'>
