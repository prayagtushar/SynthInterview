...
export const FeedbackList: React.FC<FeedbackListProps> = ({
  ...
  return (
    <div ref={feedbackRef} className="flex-1 overflow-y-auto p-4 space-y-4">
      {feedback.length === 0 ? (
        <div className="border border-dashed border-blue-500/10 rounded-xl p-6 mt-4 bg-blue-500/5">
        ...
        [...feedback].reverse().map((msg, i) => (
          <div
            key={i}
            className="group bg-slate-900/40 border border-slate-800 hover:border-blue-500/30 rounded-xl p-4 space-y-2 transition-all duration-300"
          >
            <p className="text-[11px] leading-relaxed text-slate-300 group-hover:text-slate-200 transition-colors">
              {msg}
            </p>
            <div className="flex items-center gap-2">
              <div className="h-px flex-1 bg-slate-800 group-hover:bg-blue-500/20 transition-colors" />
